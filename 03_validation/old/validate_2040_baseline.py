"""
Validate 2040 baseline with adequacy AND transmission checks.
"""
...
NETWORK_DIR = BASE / 'data' / 'networks' / '2040' / 'baseline'
RESULTS_DIR = BASE / 'results' / 'validation'
ANALYSIS_DIR = BASE / 'results' / 'analysis'

print("="*80)
print("VALIDATING 2040 BASELINE NETWORK")
print("="*80)
...
# Validation: Check expected generator count
assert len(network.generators) >= 95, f"ERROR: Expected >= 95 generators, got {len(network.generators)}"
...
if shed_fraction < 0.001:
    ...
    print(f"\nStatus: INADEQUATE - Need additional capacity")

# Extract transmission loading
print("\n" + "="*80)
...
if overloaded > 0:
    print(f"\nTop overloaded lines:")
    print(df[df['loading_pct'] >= 100][['line_id', 'bus0', 'bus1', 's_nom_MVA', 'loading_pct']].to_string(index=False))
...
results_df.to_csv(RESULTS_DIR / 'validation_2040_baseline.csv', index=False)
df.to_csv(ANALYSIS_DIR / 'line_loading_2040_baseline.csv', index=False)
...
if shed_fraction >= 0.001 and overloaded == 0:
    print("GENERATION BOTTLENECK - Need more generation capacity")
elif shed_fraction >= 0.001 and overloaded > 0:
    print("MIXED BOTTLENECK - Need both generation and transmission upgrades")
elif overloaded > 0:
    print("TRANSMISSION BOTTLENECK - Need transmission upgrades")
else:
    print("ADEQUATE - No bottlenecks")

print(f"\nResults saved to:")
print(f"  {RESULTS_DIR / 'validation_2040_baseline.csv'}")
print(f"  {ANALYSIS_DIR / 'line_loading_2040_baseline.csv'}")
print("="*80)